...
//TODO Need to add a automatic date adding feature in future for Better Refereence of Sved Score Sheet.
...
public:
    ...
    void add_run(char, int, int);     // Add Runs in Player account.
...
void Score ::add_run(char choice, int player, int ball)
{
    switch (choice)
    {
    case '0':
...
int Score ::start_scoreboard(int inning, Score t)
{
    ...
    if (inning == 1)
    {
        ...
        cout << setw(74) << "Team " << t.show_tName() << " Wons the Toss!! and Choses to Bat." << endl;
...
{
    char choice;
    outputScreen(0); //! After Fininshing change it from 1 to 0.
    do
    {
        cin >> choice;
        if (choice == '1')
